...
class Diffusion(object):
        ...
        if "ckpt_dir" in self.config.model.__dict__.keys():#获取"self.config.model"对象的所有属性和方法的名称。
            ...
            if self.config.model.model_type == 'improved_ddpm' or self.config.model.model_type == 'guided_diffusion':
                ...
                if self.config.data.dataset == "CELEBA":
                    model = torch.nn.DataParallel(model)
                    model.load_state_dict(states[0], strict=True)
                    # checkpoint = torch.load(os.path.join(self.config.model.ckpt_dir),"ckpt.pth")
                    # checkpoint = torch.load(self.config.model.ckpt_dir, map_location=self.device)
                    # checkpoint = torch.load(self.config.model.ckpt_dir, map_location=map_location)
    ...
    def sample_image(self, x, model, last=True, classifier=None, base_samples=None):
        ...
        # print('classifier_scale1=',classifier_scale)
        ...
        if self.args.sample_type == "generalized":
            ...
            # print('classifier_scale2=',classifier_scale)
            x = xs[-1]
            ...
            x = xs[-1]
        elif self.args.sample_type in ["scire", "dpm", "dpm++"]:
            ...
            def classifier_fn(x, t, y, **classifier_kwargs):#新定义的，之前的老版本没有这个地方
                logits = classifier(x, t)
                log_probs = torch.nn.functional.log_softmax(logits, dim=-1)
                return log_probs[range(len(logits)), y.view(-1)]

            # noise_schedule = NoiseScheduleVP(schedule='discrete', betas=self.betas)#获得噪声计划
            noise_schedule = NoiseScheduleVP(schedule=self.config.sampling.schedule, betas=self.betas)#获得噪声计划
            ...
            # print('classifier_scale4=',classifier_scale)
