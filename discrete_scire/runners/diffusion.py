...
class Diffusion(object):
    ...
    def sample(self):
        ...
        if "ckpt_dir" in self.config.model.__dict__.keys():  # 获取"self.config.model"对象的所有属性和方法的名称。
        ...
        if self.args.fid:
            if not os.path.exists(os.path.join(self.args.exp, "fid.npy")):
                ...
                if self.rank == 0:
                    ...
                    fid = calculate_fid_given_paths((self.config.sampling.fid_stats_dir, self.args.image_folder),
                                                    batch_size=self.config.sampling.fid_batch_size, device=self.device,
                                                    dims=2048, num_workers=8)
    ...
    def sample_fid(self, model, classifier=None):
        ...
        if total_n_samples % config.sampling.batch_size != 0:
            raise ValueError(
                "Total samples for sampling must be divided exactly by config.sampling.batch_size, but got {} and {}".format(
        ...
        if self.config.model.is_upsampling:
            base_samples_total = load_data_for_worker(self.args.base_samples, config.sampling.batch_size,
                                                      config.sampling.cond_class)
    ...
    def sample_image(self, x, model, last=True, classifier=None, base_samples=None):
        ...
        if self.config.sampling.cond_class:
            if self.args.fixed_class is None:
                ...
                classes = torch.randint(low=self.args.fixed_class, high=self.args.fixed_class + 1,
        ...
        if self.args.sample_type == "generalized":
            ...
            xs, _ = generalized_steps(x, seq, model_fn, self.betas, eta=self.args.eta, classifier=classifier,
                                      is_cond_classifier=self.config.sampling.cond_class,
                                      classifier_scale=classifier_scale, **model_kwargs)
            x = xs[-1]
            ...
            xs, _ = ddpm_steps(x, seq, model_fn, self.betas, classifier=classifier,
                               is_cond_classifier=self.config.sampling.cond_class, classifier_scale=classifier_scale,
                               **model_kwargs)
            # print('classifier_scale3=',classifier_scale)
            x = xs[-1]
        elif self.args.sample_type in ["scire", "ei"]:
            from scire_solver.scire_sampler import NoiseScheduleVP, model_wrapper, SciRE_Solver
            def model_fn(x, t, **model_kwargs):
                out = model(x, t, **model_kwargs)
                # If the model outputs both 'mean' and 'variance' (such as improved-DDPM and guided-diffusion),
                # We only use the 'mean' output for SciRE_Solver, because SciRE_Solver is based on diffusion ODEs.
                if "out_channels" in self.config.model.__dict__.keys():
                    if self.config.model.out_channels == 6:
            ...
            noise_schedule = NoiseScheduleVP(schedule=self.config.sampling.schedule, betas=self.betas)
